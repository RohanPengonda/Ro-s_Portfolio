          ...
          <div className="grid lg:grid-cols-2 gap-12">
            ...
            <div className="bg-gray-50 dark:bg-gray-700 p-8 rounded-2xl">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <button
                  type="submit"
                  className="w-full bg-gradient-to-r from-blue-600 to-purple-600 text-white py-3 px-6 rounded-lg font-semibold flex items-center justify-center gap-2 hover:shadow-lg transition-all duration-300 transform hover:scale-105"
                >
                  <Send size={20} />
                  Send Message
                </button>
                ...
                {contactInfo.map((info, index) => (
                  ...
                  >
                    <div className="text-blue-600 dark:text-blue-400 group-hover:text-blue-700 dark:group-hover:text-blue-300">
                      {iconMap[info.icon] && React.createElement(iconMap[info.icon], { className: "w-6 h-6" })}
                    </div>
                    <div>
                ...
                <div className="flex space-x-4">
                  {socialLinks.map((social, index) => (
                    <a
                      key={index}
                      href={social.url}
                      className="w-12 h-12 bg-gray-100 dark:bg-gray-700 rounded-lg flex items-center justify-center text-gray-600 dark:text-gray-400 hover:bg-blue-600 hover:text-white dark:hover:bg-blue-600 transition-all duration-300 transform hover:scale-110"
                      aria-label={social.name}
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      {iconMap[social.icon] && React.createElement(iconMap[social.icon], { className: "w-6 h-6" })}
                    </a>
                  ))}
                </div>
              </div>
